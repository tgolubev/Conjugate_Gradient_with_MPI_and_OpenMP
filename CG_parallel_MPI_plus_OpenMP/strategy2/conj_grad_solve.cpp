...
vec conj_grad_solver(const mat &A, const vec &b)
{
    ...
    size_t n = A.size();
    vec sub_x(sub_A.size()); // iniitalize a vector to store the solution subvector
    vec x(n);
    ...
    */
    ...
    vec r_old(sub_size);
    vec sub_a_times_p(sub_size); // is the size of the sub-matrix
    int max_iter = 100000;
    double alpha;
...
#pragma omp parallel for
        for (size_t i = 0; i < sub_size; i++) // loop over the rows of the sub matrix
            sub_a_times_p[i] = __gnu_parallel::inner_product(sub_A[i].begin(), sub_A[i].end(), p.begin(), 0.0);  // dot product of ith row of sub_A with the vector v


        // mpi_dot_product will perform a reduction over the sub vectors to give back the full vector!
        // THIS NEEDS TO BE DONE BY ONLY 1 OPENMP THREAD b/c does an mpi_reduce....

        alpha = mpi_dot_product(sub_r, sub_r)/std::max(mpi_dot_product(sub_p, sub_a_times_p), tolerance);

        // Next estimate of solution

        // make a parallel region and combine both for loops in it to reduce overhead of creating threads
# pragma omp parallel
        {
#pragma omp for
            for (size_t j = 0; j < sub_size; j++)
                sub_x[j] = sub_x[j] + alpha * sub_p[j];

#pragma omp for
            for (size_t j = 0; j < sub_size; j++)
                sub_r[j] = sub_r[j] - alpha * sub_a_times_p[j];  // Residual
...
#pragma omp parallel for
        for (size_t j = 0; j < sub_size; j++)
            sub_p[j] = sub_r[j] + beta * sub_p[j];  // Next gradient
